    ...
    def post(self, request):
        try:
            ...
            if serializer.is_valid():
                serializer.save()
                return response_switch("success", message="company added successfully", data=serializer.data)
            else:
...
class CompanyListAPIView(APIView):    
    ...
    def get(self, request):
        try:
            ...
            # Search filter on company name, city, state
            if search_query:
                query &= (
            ...
            company = selectList(company_table, query)
            
            paginator = CustomPagination()
            paginated_qs = paginator.paginate_queryset(company, request, view=self)

            serializer =CompanySerializer(paginated_qs, many=True)
...
class CompanyDetailAPIView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # Get the first company with status=1 created by this employee
            ...
            return response_switch(
                "bad_request",
                message="Something went wrong while retrieving the company",
                error=str(e)
            )
    ...
    def put(self, request):
        try:
            ...
            if not company_id:
                return response_switch(
                    "missing",
                    message="Company ID is required in query parameters (?id=)",
                    data=[]
                )
            ...
            serializer =CompanySerializer(instance, data=data, partial=True)
            if serializer.is_valid():
                serializer.save()
                return response_switch(
                    "success",
...
class CompanyDeleteAPIView(APIView):
    ...
    def patch(self, request):
        try:
            ...
            if not company_id:
                return response_switch("missing", message="Company ID is required", data=[])

            try:
                company_id_int = int(company_id)
            except ValueError:
                return response_switch("bad_request", message="Company ID must be an integer", data=[])

            # Filter by active company created by this user
            company = company_table.objects.filter(id=company_id_int, status=1).first()

            if not company:
                return response_switch("not_found", message="Company not found or already deleted")

            # Soft delete
            company.status = 0
            company.save()

            return response_switch("success", message=f"Company deleted successfully")

        except Exception as e:
            return response_switch("bad_request", message="Something went wrong while deleting the company", error=str(e))
